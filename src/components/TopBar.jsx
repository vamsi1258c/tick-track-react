...
import { AppBar, Toolbar, IconButton, Button, Menu, MenuItem, Typography, Avatar, Divider, Box, useMediaQuery, Drawer } from '@mui/material';
...
const TopBar = ({ isAuthenticated, onSignOut, userName, userRole }) => {
    ...
    return (
        <AppBar position="sticky" color="primary" sx={{ zIndex: 1050, boxShadow: 3, height: '55px' }}>
            <Toolbar sx={{ height: '48px', display: 'flex', justifyContent: 'space-between', width: '100%', padding: '0 16px' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    />
                    <Typography variant="h6" sx={{ fontWeight: '600', color: 'white', fontSize: '16px' }}>
                        TickTrack
                    </Typography>
                    ...
                    >
                        ...
                        <MenuItem onClick={handleDropdownMenuClose}>Another action</MenuItem>
                        <MenuItem onClick={handleDropdownMenuClose}>Something</MenuItem>
                        <Divider />
                        <MenuItem onClick={handleDropdownMenuClose}>Separated link</MenuItem>
                    </Menu>
                </Box>
                    ...
                    {isSmallScreen ? (
                        ...
                        isAuthenticated && (
                            <>
                                ...
                                >
                                    <Typography sx={{ fontWeight: 'bold', marginRight: 1 }}>{userName}</Typography>
                                ...
                                >
                                    ...
                                    <MenuItem component={Link} to={`/profile/${getLoggedInUserId()}`} sx={{ display: 'flex', alignItems: 'center' }}>
                                        <PersonIcon style={{ marginRight: '8px' }} /> View Profile
                                    </MenuItem>
                                    <MenuItem component={Link} to={`/activity/${getLoggedInUserId()}`} sx={{ display: 'flex', alignItems: 'center' }}>
                                        <ListAltIcon style={{ marginRight: '8px' }} /> View Activity Log
                                    </MenuItem>
                                    <Divider />
                                    {/* Sign Out */}
                                    <MenuItem onClick={handleSignOut} sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Drawer anchor="right" open={drawerOpen} onClose={toggleDrawer}>
                    <Box sx={{ width: 250, padding: 2 }}>
                        ...
                        {isAuthenticated ? (
                            <>
                                <Button component={Link} to={`/profile/${getLoggedInUserId()}`} sx={{ marginBottom: 2 }}>View Profile</Button>
                                <Button component={Link} to={`/activity/${getLoggedInUserId()}`} sx={{ marginBottom: 2 }}>View Activity Log</Button>
                                <Button onClick={handleSignOut}>Sign Out</Button>
                            </>
                        ) : (
                            <>
                                <Button component={Link} to="/signin" sx={{ marginBottom: 2 }}>Sign In</Button>
