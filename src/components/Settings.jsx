    ...
    .filter(
      (entry) =>
        (filterType
          ...
          : true) &&
        (filterLabel
          ? entry.label.toLowerCase().includes(filterLabel.toLowerCase())
          : true)
    )
    .sort((a, b) => {
      if (sortField === 'type') {
    ...
    <Box sx={{ padding: 3, bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ display: 'flex', marginBottom: 2, marginTop: 2, gap: 2 }}>
        ...
        >
          ...
          {[...new Set(configEntries.map((entry) => entry.type))].map(
            (type, index) => (
        ...
        >
          ...
          {[...new Set(filteredEntries.map((entry) => entry.label))].map(
            (label, index) => (
