...
const StatusDropdown = ({ currentUser, selectedTicket, onUpdateStatus }) => {
    ...
    useEffect(() => {
        ...
        const filteredOptions = statusOptions.map((option) => {
            let updatedLabel = option.label;
            if (option.value !== status) {
                if (status === 'open' && option.value === 'in_progress') updatedLabel = "Start Ticket without Approval"
                if (status === 'approved' && option.value === 'in_progress') updatedLabel = "Start Progress"
                if (option.value === "approved") updatedLabel = "Approve Ticket"
                if (option.value === "to_be_approved") updatedLabel = "Send Ticket for Approval"
            ...
            (option) => validTransitions.includes(option.value) || option.value === status
        )
    ...
    return (
        <FormControl size="small" sx={{ m: 1, minWidth: 120 }}>
            ...
            >
                {availableStatuses
                    .map((statusOption) => (
                        <MenuItem key={statusOption.value} value={statusOption.value} disabled={statusOption.value === status} >
                            {statusOption.label}
                        </MenuItem>
                    ))}
