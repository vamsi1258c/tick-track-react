...
import {
    Container,
    Grid, IconButton,
    TextField, Button, Alert, Card, CardHeader, Typography,
    CardContent, useMediaQuery, Box
} from '@mui/material';
...
const TicketCreate = ({ currentUser }) => {
    ...
    useEffect(() => {
        const fetchAllUsers = async () => {
            try {
                ...
                setUsers(response.data.map(user => ({ id: user.id, label: user.username })));
    ...
    const validateFields = useCallback(() => {
        ...
        if (!ticketData.description.trim()) errors.description = 'Description is required';
        ...
        <Container maxWidth="md">
            <Card variant="contained" sx={{ mt: 0, p: '3px 4px', borderRadius: 1 }}>
                ...
                <CardContent sx={{ p: 1 }}>
                    ...
                    {success && <Alert severity="success" sx={{ mb: 1 }}>Ticket created successfully!</Alert>}
                        ...
                        <Grid container spacing={1} alignItems="center">
                            ...
                            <Grid item xs={12} sx={{ mt: 2 }}>
                                ...
                                {touchedFields.description && validationErrors.description && (
                                    <Typography color="error" sx={{ mt: 0.5, fontSize: '0.75rem' }}>{validationErrors.description}</Typography>
                                )}
                            </Grid>
                            ...
                            <Grid item xs={12}>
                                ...
                                >
                                    ...
                                    <Grid item xs={12} sm={3}>
                                        <Autocomplete
                                            value={categoryOptions.find(option => option.value === ticketData.category) || null}
                                            onChange={(event, newValue) => handleCategoryChange({ target: { name: 'category', value: newValue?.value } })}
                                            options={categoryOptions}
                                            getOptionLabel={(option) => option.label}
                                            isOptionEqualToValue={(option, value) => option.value === value}
                                            renderInput={(params) => (
                                                <TextField {...params} label="Category" variant="outlined" size="small" margin="dense" />
                                            )}
                                            sx={{
                                                minWidth: 150,
                                                '& .MuiInputBase-root': {
                                    ...
                                    <Grid item xs={12} sm={3}>
                                        <Autocomplete
                                            value={subcategories.find(option => option.value === ticketData.subcategory) || null}
                                            onChange={(event, newValue) => handleChange({ target: { name: 'subcategory', value: newValue?.value } })}
                                            options={subcategories}
                                            getOptionLabel={(option) => option.label}
                                            isOptionEqualToValue={(option, value) => option.value === value}
                                            renderInput={(params) => (
                                                <TextField {...params} label="Subcategory" variant="outlined" size="small" margin="dense" />
                                            )}
                                            sx={{
                                                minWidth: 150,
                                                '& .MuiInputBase-root': {
                                    ...
                                    <Grid item xs={12} sm={3}>
                                        <Autocomplete
                                            value={priorityOptions.find(option => option.value === ticketData.priority) || null}
                                            onChange={(event, newValue) => handleChange({ target: { name: 'priority', value: newValue?.value } })}
                                            options={priorityOptions}
                                            getOptionLabel={(option) => option.label}
                                            isOptionEqualToValue={(option, value) => option.value === value}
                                            renderInput={(params) => (
                                                <TextField {...params} label="Priority" variant="outlined" size="small" margin="dense" />
                                            )}
                                            sx={{
                                                minWidth: 150,
                                                '& .MuiInputBase-root': {
                                    ...
                                    <Grid item xs={12} sm={3}>
                                        <Autocomplete
                                            options={users}
                                            getOptionLabel={(option) => option.label}
                                            value={users.find(user => user.id === ticketData.assigned_to)}
                                            onChange={handleAssignedToChange}
                                            renderInput={(params) => (
                                                <TextField {...params} label="Assignee" variant="outlined" size="small" margin="dense" sx={{ minWidth: 150 }} />
                                            )}
                                        />
                                    </Grid>
                                </Grid>
                            ...
                            <Grid item xs={12} sx={{ mt: 2 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
                                    ...
                                    <Button
                                        variant="outlined"
                                        component="label"
                                        sx={{ padding: '2px 6px', fontSize: '0.7rem', borderRadius: 0.5 }}
                                        size="small"
                                    >
                            ...
                            {/* Buttons Section */}
                            <Grid item xs={12} sx={{ mt: 1 }} container spacing={2} justifyContent="flex-start">
                                <Grid item>
                                    <Button type="submit" variant="contained" size="small" color="primary" sx={{ width: '80px' }} disabled={!isFormValid}>
                                        Submit
                                    </Button>
                                </Grid>
                                <Grid item>
                                    <Button variant="outlined" size="small" onClick={clearForm} sx={{ width: '80px' }}>
                                        Clear
                                    </Button>
                                </Grid>
                                <Grid item>
                                    <Button variant="outlined" size="small" color="secondary" onClick={() => navigate(-1)} sx={{ width: '80px' }}>
                                        Cancel
                                    </Button>
                                </Grid>
                            </Grid>
