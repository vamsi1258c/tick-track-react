...
const CommentModal = ({ show, onClose, onSubmit, approvers, loading, ticketId, currentUserId, status, newStatus }) => {
    ...
    return (
        <Modal show={show} onHide={onClose} maxWidth="sm" fullWidth>
            <DialogTitle sx={{ backgroundColor: 'primary.main', color: 'white', fontWeight: 'bold' }}><Box display="flex" alignItems="center" justifyContent="center" my={3} >
                ...
                {loading ? (
                    ...
                    <> {newStatus === 'to_be_approved' && (
                        <FormControl fullWidth variant="outlined" sx={{ my: 2 }}>
                            <InputLabel id="approver-select-label">Select an approver</InputLabel>
                            <Select
